...
public class HardDKLevel extends Level {
...
//        Painter.fill(this, arena, 6, Terrain.SIGN);
    ...
    public int getSummoningPos(){
        Mob king = getKing();
        HashSet<DwarfKingHard.Summoning> summons = king.buffs(DwarfKingHard.Summoning.class);
        ArrayList<Integer> positions = new ArrayList<>();
        for (int pedestal : pedestals) {
            boolean clear = true;
            for (DwarfKingHard.Summoning s : summons) {
    ...
    private Mob getKing(){
        for (Mob m : mobs){
            if (m instanceof DwarfKingHard) return m;
        }
        return null;
    }
    ...
    public void seal() {
        ...
        DwarfKingHard boss = new DwarfKingHard();
